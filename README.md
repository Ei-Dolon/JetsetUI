...
01. The Jetset environment DApp's have the same fixed DApp Header and Footer components:   
 
02. Header shows the animated Jetset Logo in the center of the header and a Settings Icon cog in the top right of the header, AppHeader z=100 x=360 y=140 fixed to top of viewport.  

03. Footer z=100 x=360 y=100 fixed to bottom of viewport. Footer displays the main user interaction interface as an animated connect wallet button.  
...
04. The central main content area z=50 y=360 has no horizontal scroll, but can vertically scroll between the header and footer if neccessary and it's contents rely on a responsive design to display correctly for the device being used.  
 
05. The user can then interact with the DApp, via onClick handlers of which currently clickable elements include the "Connect Wallet" button (btnConnect.png), the Disconnect Wallet button (btnDisconnect.png), the Settings Icon (settings.svg), once a wallet is successfully connected there will also be a Buy button, Receive button and TipTap button.  
 
06. Once the user has successfully connected an installed wallet, the DApp displays a wallet header with info on the connected wallet. 
 
07. The wallets BNB Smart Chain portfolio values are then display using wagmi useBalance to read BNB in wallet, and useReadContract to read the Jetset contract for the connected wallets address for the number of Jetset tokens held.
 
08. These balances are displayed along with their fiat currency value in the selected fiat currency, defaults to usd ($), as provided by priceData.json  
 
09. The total sum of the cryptocurrency values in fiat currency is also displayed.  
...
Added: Modified the TanStack Query cache by adding Tanstack LocalStorage Persister plugin, this syncs the in-memory cache directly to localStorage by using createSyncStoragePersister.  This will allow dapps that have dropped connection to still function.  

Improvement: Changed action of cache by adding automatic refetchInterval set to 10 minutes, so data will be  flagged as stale after 10 minutes and refetches after 10 minutes causing a rerender due to cache change, so latest prices should always be close to 10 minutes old at most, disregarding any error conditions.  
